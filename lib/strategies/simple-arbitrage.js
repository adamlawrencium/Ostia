...
var outputPerformanceData = function(exchangeData,performanceData) {
  ...
  for (var exchange in exchangeData) {
    if (exchangeData.hasOwnProperty(exchange)) {
      console.log(exchange + ":\t" +  parseFloat(exchangeData[exchange].highbid).toPrecision(4) + ", " +
                                      parseFloat(exchangeData[exchange].highbid_amt).toPrecision(4) + "\t|| " +
                                      parseFloat(exchangeData[exchange].lowask).toPrecision(4) + ", " +
                                      parseFloat(exchangeData[exchange].lowask_amt).toPrecision(4));
    }
  }
  ...
  // Placing each summarized orderbook in the data structure exchSumm
    ...
    for (var exchange in exchSumm) {
      if (exchSumm.hasOwnProperty(exchange)) {
        if (exchSumm[exchange].highbid > highest_bid) {
          highest_bid     = exchSumm[exchange].highbid;
          highest_bid_amt = exchSumm[exchange].highbid_amt;
          order[0].exchange = exchange;

        }
        if (exchSumm[exchange].lowask < lowest_ask) {
          lowest_ask      = exchSumm[exchange].lowask;
          lowest_ask_amt  = exchSumm[exchange].lowask_amt;
...
// Export
module.exports = {
  strategy, //For standardizing strategy calls
  order
}
