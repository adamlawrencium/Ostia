...
async.waterfall([
  ...
  function (callback) {
    ...
    var seriesLength = smallerWindowSMA.length;
    ...
    var switched = false;
    for (var i = 0; i < largerWindowSMA.length; i++) {
      console.log(i);
      ...
      if (!switched) {
        if (lowerIndicator[i][1] > upperIndicator[i][1]) {
          flagsArray.push([lowerIndicator[i][0], lowerIndicator[i][1]]);
