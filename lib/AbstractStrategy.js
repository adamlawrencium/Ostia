...
class AbstractStrategy {
  ...
  createTradeOrderObjFromState(currencyPair, newState, timestamp) {
    var closingPrice = getClosingPriceFromTimestamp(this.tickerData, timestamp);
  ...
  driver() {
    ...
    for (var i = 0; i < seriesLength; i++) {
      currentState = this.determineStrategyState(i, smallerWindowSMA, largerWindowSMA);
      if (currentState != previousState) {

        var tradeTimestamp = smallerWindowSMA[i][0]; // timestamp of trade

        var order = this.createTradeOrderObjFromState(A_B, currentState, tradeTimestamp);
