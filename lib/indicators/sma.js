...
poloniex.returnChartData("USDT", "ETH", 86400, startDate, endDate, function(err, data) {
  if (err) {
    console.log("error!");
  }
  console.log(data);
  for (var i = 0; i < data.length; i++) {
    timeData.push([data[i].date, data[i].close]);
  }

  for (var i = 0; i < timeFrame; i++) {   // add elements to SMA
    var sum = 0;
    console.log('sum: '+sum);
    for (var j = 0; j < timeFrame; j++) {   // go through
      sum += timeData[j][1];
      console.log('tempsum: '+sum);
    }
    console.log('finalsum: '+sum);
    sum = sum / timeFrame;
    console.log('finalavg: '+sum);
    SMATimeSeries.push([timeData[i+timeFrame][0], sum]);
