...
/*
    ...
    console.log(orderbook);
      console.log(mapParse(orderbook));
*/
  ...
  switch (data.type) {
    case "open":
      if(orderbook.hasOwnProperty(key)){
        orderbook[key] += mult * parseFloat(data.remaining_size);
      }
      else {
        orderbook[key] = mult * parseFloat(data.remaining_size);
      }
    ...
    case "done":
      if (orderbook.hasOwnProperty(key)){
        ...
        orderbook[key] = tmp - (mult * parseFloat(data.remaining_size));
        if ((mult * orderbook[key]) < min){
          delete(orderbook[key]);
        }
      }
      break;
    case "match":
      if (orderbook.hasOwnProperty(key)){
        var tmp = parseFloat(orderbook[key]);
        //console.log(orderbook[key] + ", " + tmp);

        delete(orderbook[key]);
        orderbook[key] = tmp - (mult * parseFloat(data["size"]));

        //console.log(mult + ", " + parseFloat(data.size) + ", " + (tmp - (mult * parseFloat(data["size"]))));
        //console.log(orderbook[key]);
    ...
    case "change":
      if (orderbook.hasOwnProperty(key)){
        ...
        orderbook[key] = tmp - (mult * (parseFloat(data.old_size) - parseFloat(data.new_size)));
      }
      break;
    // These message are for level 4 orderbooks, we only need level 2
    default:
...
function openWebSocket(pair) {
  ...
  var url = "https://api.gdax.com/products/"+ pair + "/book?level=2";
  var url2 = "https://api.gdax.com/products/"+ pair + "/book?level=1";
  var options = {
    url: url,
    headers: {
      'User-Agent': 'request'
