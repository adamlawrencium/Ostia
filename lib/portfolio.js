  ...
  calculateHistoricalPerformance(candlestickData, currencyPair, metric) {
    ...
    while (posCounter < numPositions) {
      while (tickCounter < candlestickData.length) {
        if (candlestickData[tickCounter].date == ) {
    ...
    for (var i = 0; i < numPositions; i++) {
      for (var j = 0; j < candlestickData.length; j++) {
        var curPos = this.positions[currencyPair].positionLog[i];
        var tsamp = candlestickData[i].date;
        var portVal = this.positions[currencyPair].quantity * candlestickData[i].close;
        backtest.push([candlestickData[i].date, ])
