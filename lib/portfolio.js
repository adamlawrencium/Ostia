module.exports = class Portfolio {
  ...
  calculateHistoricalPerformance(candlestickData, currencyPair, metric) {
    ...
    // var posCounter = 0;
    // var tickCounter = 0;
    // while (posCounter < numPositions) {
    //   while (tickCounter < candlestickData.length) {
    //     // if the current ticker matches up with a trade order, then adjust the portfolio value accordingly.
    //     var currentTicker = candlestickData[tickCounter].date;
    //     var tradeOrder = this.positions[currencyPair].positionLog[posCounter].
    //     if (candlestickData[tickCounter].date == this.positions[currencyPair].positionLog[posCounter].timestamp) {
    //       var portVal = this.positions[currencyPair].positionLog[posCounter].quantity * candlestickData[tickCounter].close;
    //       backtest.push([candlestickData[tickCounter].date, portVal]);
    //       posCounter++;
    //     }
    //     else {
    //       var portVal = this.positions[currencyPair].positionLog[posCounter].quantity * candlestickData[tickCounter].close;
