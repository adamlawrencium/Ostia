module.exports = class Portfolio {
  ...
  */
  ...
  calculateHistoricalPerformance(candlestickData, currencyPair, metric) {
    ...
    // Looping through USD-BTC data, looking at each date-close pair
    for (var i = 0; i < candlestickData.length; i++) {
      ...
      for (var j = 0; j < numPositions; j++) {
        ...
        if (candlestickData[i].date == positionLog[j].timestamp) {
          ...
          // If BUY, use all USD to purchase BTC, using current exchange rate
          if(positionLog[j].longShort == 'BUY'){
            BTC = USD/candlestickData[i].close;
            USD = 0;
          }
          // If SELL, use all BTC to purchase USD, using current exchange rate
          else {
            // If we don't have any BTC, that means we havent executed a BUY yet, so continue
            if(BTC == 0){
              continue;
            }
            USD = BTC*candlestickData[i].close;
            BTC = 0;
          }

          // Pushing the portfolio value and timestamp after executing a position
          currentPortfolioValue = USD + BTC * candlestickData[i].close;
          backtest.push([candlestickData[i].date, currentPortfolioValue]);
        }
        else {
