...
class DataHandler {
  ...
  generateIndicatorTimeSeries(tickerData) {
    this.generatedIndicators = [];
    for (var i = 0; i < this.reqIndicators.length; i++) {
      console.log(i);
      console.log(this.reqIndicators[i]);
    }
    return "This is the final output of the indicators"
  }
  ...
  getFinancialData() {
    return new Promise( (resolve, reject) => {
      ...
      .then( data => {
        resolve(this.generateIndicatorTimeSeries(data));
      })
      .catch( err => {
        reject(err);
      });
      // reject('Could not get financialData');
    });
  }
